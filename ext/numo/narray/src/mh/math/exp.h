#ifndef NUMO_NARRAY_MH_EXP_H
#define NUMO_NARRAY_MH_EXP_H 1

#define DEF_NARRAY_FLT_EXP_METHOD_FUNC(tDType, tNAryType)                                      \
  static void iter_##tDType##_math_s_exp(na_loop_t* const lp) {                                \
    size_t n;                                                                                  \
    char *p1, *p2;                                                                             \
    ssize_t s1, s2;                                                                            \
    size_t *idx1, *idx2;                                                                       \
      ...
      if (idx2) {                                                                              \
        ...
        for (size_t i = 0; i < n; i++) {                                                       \
          ...
          SET_DATA_STRIDE(p2, s2, tDType, x);                                                  \
                                                                                               ...
                                                                                               \
  static VALUE tDType##_math_s_exp(VALUE mod, VALUE a1) {                                      \
    ndfunc_arg_in_t ain[1] = { { tNAryType, 0 } };                                             \
    ndfunc_arg_out_t aout[1] = { { tNAryType, 0 } };                                           \
    ndfunc_t ndf = { iter_##tDType##_math_s_exp, FULL_LOOP, 1, 1, ain, aout };                 \
    return na_ndloop(&ndf, 1, a1);                                                             \
  }

#endif /* NUMO_NARRAY_MH_EXP_H */
