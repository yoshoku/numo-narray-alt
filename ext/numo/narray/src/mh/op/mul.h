...
#define ITER_MUL_INPLACE_OR_NEW_P2_ARY(tDType)                                                 \
  if (p1 == p3) {                                                                              \
    for (size_t i = 0; i < n; i++) {                                                           \
      ((tDType*)p1)[i] = m_mul(((tDType*)p1)[i], ((tDType*)p2)[i]);                            \
    }                                                                                          \
  } else {                                                                                     \
    for (size_t i = 0; i < n; i++) {                                                           \
      ((tDType*)p3)[i] = m_mul(((tDType*)p1)[i], ((tDType*)p2)[i]);                            \
    }                                                                                          \
  }

#define ITER_MUL_INPLACE_OR_NEW_P2_SCL(tDType)                                                 \
  if (p1 == p3) {                                                                              \
    for (size_t i = 0; i < n; i++) {                                                           \
      ((tDType*)p1)[i] = m_mul(((tDType*)p1)[i], *(tDType*)p2);                                \
    }                                                                                          \
  } else {                                                                                     \
    for (size_t i = 0; i < n; i++) {                                                           \
      ((tDType*)p3)[i] = m_mul(((tDType*)p1)[i], *(tDType*)p2);                                \
    }                                                                                          \
  }

#define ITER_MUL_INPLACE_OR_NEW_PTR_P2_ARY(tDType)                                             \
  if (p1 == p3) {                                                                              \
    for (size_t i = 0; i < n; i++) {                                                           \
      *(tDType*)p1 = m_mul(*(tDType*)p1, *(tDType*)p2);                                        \
      p1 += s1;                                                                                \
      p2 += s2;                                                                                \
    }                                                                                          \
  } else {                                                                                     \
    for (size_t i = 0; i < n; i++) {                                                           \
      *(tDType*)p3 = m_mul(*(tDType*)p1, *(tDType*)p2);                                        \
      p1 += s1;                                                                                \
      p2 += s2;                                                                                \
      p3 += s3;                                                                                \
    }                                                                                          \
...
#define ITER_MUL_NEW_PTR_P2_SCL(tDType)                                                        \
  for (size_t i = 0; i < n; i++) {                                                             \
    *(tDType*)p3 = m_mul(*(tDType*)p1, *(tDType*)p2);                                          \
    p1 += s1;                                                                                  \
    p3 += s3;                                                                                  \
...
#define ITER_MUL_FALLBACK_LOOP(tDType)                                                         \
  for (size_t i = 0; i < n; i++) {                                                             \
    tDType x;                                                                                  \
    tDType y;                                                                                  \
    tDType z;                                                                                  \
...
#define DEF_MUL_SELF_FUNC(tDType, tNAryClass)                                                  \
  static VALUE tDType##_mul_self(VALUE self, VALUE other) {                                    \
    ndfunc_arg_in_t ain[2] = { { tNAryClass, 0 }, { tNAryClass, 0 } };                         \
    ndfunc_arg_out_t aout[1] = { { tNAryClass, 0 } };                                          \
    ndfunc_t ndf = { iter_##tDType##_mul, STRIDE_LOOP, 2, 1, ain, aout };                      \
    return na_ndloop(&ndf, 2, self, other);                                                    \
  }

/* Common macro for add function */
#define DEF_MUL_FUNC(tDType, tNAryClass)                                                       \
  static VALUE tDType##_mul(VALUE self, VALUE other) {                                         \
    VALUE klass = na_upcast(rb_obj_class(self), rb_obj_class(other));                          \
    if (klass == tNAryClass) {                                                                 \
      return tDType##_mul_self(self, other);                                                   \
    } else {                                                                                   \
  ...
  static void iter_##tDType##_mul(na_loop_t* const lp) {                                       \
    ...
    if (is_aligned(p1, sizeof(tDType)) && is_aligned(p2, sizeof(tDType)) &&                    \
      ...
      if (s1 == sizeof(tDType) && s2 == sizeof(tDType) && s3 == sizeof(tDType)) {              \
        ITER_MUL_INPLACE_OR_NEW_P2_ARY(tDType)                                                 \
        return;                                                                                \
      }                                                                                        \
      if (is_aligned_step(s1, sizeof(tDType)) && is_aligned_step(s2, sizeof(tDType)) &&        \
        ...
        if (s2 == 0) {                                                                         \
          if (s1 == sizeof(tDType) && s3 == sizeof(tDType)) {                                  \
            ITER_MUL_INPLACE_OR_NEW_P2_SCL(tDType)                                             \
          } else {                                                                             \
            ITER_MUL_NEW_PTR_P2_SCL(tDType)                                                    \
          }                                                                                    \
        } else {                                                                               \
          ITER_MUL_INPLACE_OR_NEW_PTR_P2_ARY(tDType)                                           \
        }                                                                                      \
        return;                                                                                \
      }                                                                                        \
    }                                                                                          \
    ITER_MUL_FALLBACK_LOOP(tDType)                                                             \
  }                                                                                            \
                                                                                               \
  DEF_MUL_SELF_FUNC(tDType, tNAryClass)                                                        \
  DEF_MUL_FUNC(tDType, tNAryClass)
  ...
  static void iter_robject_mul(na_loop_t* const lp) {                                          \
    ...
    if (s2 == 0) {                                                                             \
      if (s1 == sizeof(robject) && s3 == sizeof(robject)) {                                    \
        ITER_MUL_INPLACE_OR_NEW_P2_SCL(robject)                                                \
      } else {                                                                                 \
        ITER_MUL_NEW_PTR_P2_SCL(robject)                                                       \
      }                                                                                        \
    } else {                                                                                   \
      ITER_MUL_INPLACE_OR_NEW_PTR_P2_ARY(robject)                                              \
    }                                                                                          \
  }                                                                                            \
                                                                                               \
  DEF_MUL_SELF_FUNC(robject, numo_cRObject)                                                    \
  ...
  static void iter_sfloat_mul(na_loop_t* const lp) {                                           \
    ...
    if (is_aligned(p1, sizeof(sfloat)) && is_aligned(p2, sizeof(sfloat)) &&                    \
      ...
      if (s1 == sizeof(sfloat) && s2 == sizeof(sfloat) && s3 == sizeof(sfloat)) {              \
        if ((n >= num_pack) &&                                                                 \
          ...
          if (p1 == p3) {                                                                      \
            for (i = 0; i < cnt; i++) {                                                        \
              ((sfloat*)p1)[i] = m_mul(((sfloat*)p1)[i], ((sfloat*)p2)[i]);                    \
            }                                                                                  \
          } else {                                                                             \
            for (i = 0; i < cnt; i++) {                                                        \
              ((sfloat*)p3)[i] = m_mul(((sfloat*)p1)[i], ((sfloat*)p2)[i]);                    \
            }                                                                                  \
          }                                                                                    \
                                                                                               \
        ...
        if (s2 == 0) {                                                                         \
          if (s1 == sizeof(sfloat) && s3 == sizeof(sfloat)) {                                  \
            ...
            if ((n >= num_pack) &&                                                             \
              ...
              if (p1 == p3) {                                                                  \
                for (; i < cnt; i++) {                                                         \
                  ((sfloat*)p1)[i] = m_mul(((sfloat*)p1)[i], *(sfloat*)p2);                    \
                }                                                                              \
              } else {                                                                         \
                for (; i < cnt; i++) {                                                         \
                  ((sfloat*)p3)[i] = m_mul(((sfloat*)p1)[i], *(sfloat*)p2);                    \
                }                                                                              \
              }                                                                                \
                                                                                               \
              cnt_simd_loop = (n - i) % num_pack;                                              \
            ...
            for (i = 0; i < n; i++) {                                                          \
              *(sfloat*)p3 = m_mul(*(sfloat*)p1, *(sfloat*)p2);                                \
              p1 += s1;                                                                        \
              p3 += s3;                                                                        \
            }                                                                                  \
          }                                                                                    \
          ...
          if (p1 == p3) {                                                                      \
            for (i = 0; i < n; i++) {                                                          \
              *(sfloat*)p1 = m_mul(*(sfloat*)p1, *(sfloat*)p2);                                \
              p1 += s1;                                                                        \
              p2 += s2;                                                                        \
            }                                                                                  \
          } else {                                                                             \
    ...
    for (i = 0; i < n; i++) {                                                                  \
      ...
      SET_DATA_STRIDE(p3, s3, sfloat, z);                                                      \
                                                                                               ...
                                                                                               \
  DEF_MUL_SELF_FUNC(sfloat, numo_cSFloat)                                                      \
  DEF_MUL_FUNC(sfloat, numo_cSFloat)
  ...
  static void iter_dfloat_mul(na_loop_t* const lp) {                                           \
    ...
    for (i = 0; i < n; i++) {                                                                  \
      ...
      SET_DATA_STRIDE(p3, s3, dtype, z);                                                       \
                                                                                               ...
                                                                                               \
  DEF_MUL_SELF_FUNC(dfloat, numo_cDFloat)                                                      \
  DEF_MUL_FUNC(dfloat, numo_cDFloat)
